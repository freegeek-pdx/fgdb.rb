module ConditionsHelper
  ...
  def html_for_job_condition(params_key)
    ...
    select(params_key, "job_id", jobs.sort_by(&:name).collect {|p| [ p.name, p.id ] }, {}, _multi_html_opts.merge(:onkeyup => "magic_onkeyscroll(event);"))
  end
  ...
  def html_for_weekday_condition(params_key)
    return Weekday.find(:all).sort_by(&:id).collect{|p| [p.name, p.id]}.map{|x| '<div style="padding: 10px; display: inline-block; float: left;' + (x.last == 0 ? " clear: left;" : "") + '">' + label(params_key, "weekday_id[#{x.last}]", x.first) + check_box(params_key, "weekday_id", {:id => params_key + "_weekday_id[#{x.last}]", :name => params_key + "[weekday_id][]"}, x.last, "") + '</div>'}.join("")

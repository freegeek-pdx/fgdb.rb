    ...
    @txn.receipt_types.each do |receipt_type|
      ...
      if receipt_type == 'receipt'
        ...
        if fees_ge.length > 0
          ...
          data += @txn.gizmo_events.select{|x| x.is_fee_line?}.map{|x| {"qty" => x.gizmo_count, "desc" => x.attry_processing_description, "val" => my_number_to_currency(x.gizmo_count * x.unit_price_cents)}}
